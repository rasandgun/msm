...
def compile_program(src_path: str) -> List[str]:
    ...
    out_path = os.path.join(base_dir, f"msm_{name}")

    if compiler is not None:
        comp_args = [arg.format(src=src_path, out=out_path, dir=base_dir, name=name)
    ...
    if isinstance(run_template, list):
        return [arg.format(src=src_path, out=out_path, dir=base_dir, name=name)
                for arg in run_template]
    else:
        return [run_template.format(src=src_path, out=out_path, dir=base_dir, name=name)]
...
def run_program(cmd: List[str], input_data: str = "", timeout: float = 5.0) -> subprocess.CompletedProcess:
    try:
        ...
        raise RuntimeError(f"Runtime error (code {e.returncode}): {e.stderr or e.stdout}")