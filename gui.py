...
class TesterGUI:
    ...
    def setup_ui(self):
        ...
        self.brute_label.grid(row=1, column=0, sticky=tk.W, pady=5)
        self.brute_var = tk.StringVar()
        self.brute_entry = ttk.Entry(file_frame, textvariable=self.brute_var)
        self.brute_entry.grid(row=1, column=1, sticky=(tk.W, tk.E), pady=5, padx=(5, 0))
        self.brute_button = ttk.Button(file_frame, text="Browse", command=lambda: self.browse_file(self.brute_var))
        self.brute_button.grid(row=1, column=2, padx=(5, 0))
        
        self.generator_label = ttk.Label(file_frame, text="Generator:")
        self.generator_label.grid(row=2, column=0, sticky=tk.W, pady=5)
        self.generator_var = tk.StringVar()
        self.generator_entry = ttk.Entry(file_frame, textvariable=self.generator_var)
        self.generator_entry.grid(row=2, column=1, sticky=(tk.W, tk.E), pady=5, padx=(5, 0))
        self.generator_button = ttk.Button(file_frame, text="Browse", command=lambda: self.browse_file(self.generator_var))
        self.generator_button.grid(row=2, column=2, padx=(5, 0))
    ...
    def on_mode_change(self, event=None):
        ...
        if mode == "Brute Force":
            ...
            self.generator_entry.grid()
            self.generator_button.grid()
        else:
            self.brute_label.config(text="Interactor:")
            self.generator_label.grid_remove()
            self.generator_entry.grid_remove()
            self.generator_button.grid_remove()
    def run_tests(self):
        if self.running:
            return
        ...
        if mode == "Brute Force":
            ...
            if not all([solution_path, brute_path]):
                messagebox.showerror("Error", "Please select Solution and Interactor files")
                return
        ...
        thread = threading.Thread(
            ...
            args=(solution_path, brute_path, generator_path, num_tests, mode)
        )
        thread.daemon = True
        thread.start()
    
    def _run_tests_thread(self, solution_path, brute_path, generator_path, num_tests, mode):
        try:
            self.log_console("Initializing tester...")
            if mode == "Brute Force":
                self.tester = BruteForceTester(solution_path, brute_path, generator_path, self.stop_event)
            else:
                self.tester = InteractiveTester(solution_path, brute_path, self.stop_event)
            ...
            self.log_console(f"Tests run: {num_tests}{stopped}")
